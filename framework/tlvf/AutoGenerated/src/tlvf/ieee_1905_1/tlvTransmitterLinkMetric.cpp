...
std::shared_ptr<tlvTransmitterLinkMetric> tlvTransmitterLinkMetric::castFrom(std::shared_ptr<BaseClass> source) {
    return std::make_shared<tlvTransmitterLinkMetric>(source->getStartBuffPtr(),source->getLen()+source->getBuffRemainingBytes(),true);
}
